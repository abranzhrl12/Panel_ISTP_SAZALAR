...
const menuItems: MenuItem[] = [
  ...
  { id: 'reporting', label: 'Reporting', icon: 'BarChart3', children: [
    { id: 'check-ins', label: 'Check-ins', icon: 'CheckSquare', href: '/reporting/check-ins' },
    { id: 'objectives', label: 'Objectives', icon: 'Target', href: '/reporting/objectives' },
    { id: 'career-hub', label: 'Career Hub', icon: 'Briefcase', href: '/reporting/career' }
  ]},
...
const iconMap: Record<string, React.ComponentType<any>> = {
  Home, Search, BarChart3, CheckSquare, Target, Briefcase, Bell, Mail,
  Inbox, Calendar, ListTodo, FileText, HelpCircle, Settings, ExternalLink
};
...
const AppPrueba: React.FC<SidebarProps> = ({ isMobile }) => {
    ...
    return (
      <div key={item.id}>
        ...
        <button
          ...
          className={`w-full flex items-center ${isCollapsed && !isMobile ? 'justify-center px-0' : 'gap-3 px-3'} py-2.5 rounded-lg text-sm font-medium transition-all duration-300 group relative ${level > 0 ? 'ml-6 pl-6' : ''} ${isActive ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/25' : 'text-gray-600 hover:text-gray-900 hover:bg-gray-100'}`}
        >
          <Icon size={20} className="flex-shrink-0" />
          {(!isCollapsed || isMobile) && (
            <span className="flex-1 text-left whitespace-nowrap overflow-hidden transition-all duration-300">
              {item.label}
              {item.badge && (<span className="ml-2 bg-blue-600 text-white text-xs px-2 py-0.5 rounded-full font-semibold">{item.badge}</span>)}
            </span>
          )}
          {(!isCollapsed || isMobile) && hasChildren && (<div className="transition-transform duration-200 flex-shrink-0">{isExpanded ? <ChevronDown size={16} /> : <ChevronRight size={16} />}</div>)}
          {isCollapsed && !isMobile && item.badge && (<div className="absolute -top-1 -right-1 w-3 h-3 bg-blue-600 rounded-full"></div>)}
        </button>
        {hasChildren && (!isCollapsed || isMobile) && (
          <div className={`mt-1 space-y-1 overflow-hidden transition-[max-height] duration-300 ease-in-out ${isExpanded ? 'max-h-screen' : 'max-h-0'}`}>
  ...
  return (
    <>
      ...
      <div className={`fixed left-0 top-0 h-screen z-50 lg:relative lg:z-auto bg-white border-gray-200 border-r transition-all duration-300 ease-in-out ${isCollapsed && !isMobile ? 'w-16' : 'w-72'} ${isMobile ? (isCollapsed ? '-translate-x-full' : 'translate-x-0') : 'translate-x-0'} flex flex-col`}>
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-inherit flex-shrink-0">
          <div className={`flex items-center gap-3 ${isCollapsed && !isMobile ? 'justify-center' : ''}`}>
            ...
            <h1 className={`font-bold text-lg text-gray-900 overflow-hidden whitespace-nowrap transition-all duration-300 ${isCollapsed && !isMobile ? 'max-w-0 opacity-0' : 'max-w-full opacity-100'}`}>
              Beyond UI
            </h1>
          </div>
        </div>
        ...
        {/* Navigation sections */}
        <div className="flex-1 p-4 space-y-2 overflow-y-auto" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
        ...
        {/* User Profile */}
        <div className={`p-4 border-t border-inherit flex-shrink-0 ${isCollapsed && !isMobile ? 'px-2' : ''}`}>
          <div className="flex items-center gap-3">
            <img src={user.avatar} alt={user.name} className="w-8 h-8 rounded-full object-cover flex-shrink-0" />
            <div className={`flex-1 min-w-0 overflow-hidden transition-all duration-300 ${isCollapsed && !isMobile ? 'max-w-0 opacity-0' : 'max-w-full opacity-100'}`}>
