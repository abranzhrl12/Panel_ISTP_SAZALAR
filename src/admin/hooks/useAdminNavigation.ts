// src\admin\hooks\useAdminNavigation.ts
...
export function useAdminNavigation(
  ...
  const [adminNavigationItems, setAdminNavigationItems] = useState<
    AdminNavigationItem[]
  >([]);
  ...
  const fetchNavItems = useCallback(
    async (currentOptions?: UseAdminNavigationOptions) => {
  ...
  const refetch = useCallback(
    async (refetchOptions?: UseAdminNavigationOptions) => {
    ...
    [fetchNavItems, options]
  ); // Dependencias: la función memoizada y las opciones actuales
